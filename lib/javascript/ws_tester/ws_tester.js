    ...
    return {
        ...
        execute_tests:                function execute_tests() {            
            return $.ajax({
                ...
                dataType:    'json',
                cache:        false,
                data:        {action: 'execute_tests', application: $('#test_applications option:selected').val(), test_type: $('#test_test_types option:selected').val()}
            });
        },
        
        request_results:            function request_results() {
            var selected_tab = $("#tabs ul.ui-tabs-nav li.ui-state-active a").text();
            var selected_application = (selected_tab == 'Test' ?  $('#test_applications option:selected').val() :  $('#review_applications option:selected').val());
            var selected_test_type = (selected_tab == 'Test' ? $('#test_test_types option:selected').val() : $('#review_test_types option:selected').val());

            if (selected_tab == 'Review') {
                var selected_test_epoch = $('#run_dates option:selected').val();
                usbr.ws_tester.result_file_path = usbr.ws_tester.result_file_path = usbr.ws_tester.document_root + '/results/' + selected_application + '.' + selected_test_type + '.' + selected_test_epoch + '.result'
            }
            ...
            return $.ajax({
                ...
                dataType:    'json',
                cache:        false,
                data:        {action: 'request_results', application: selected_application, test_type: selected_test_type, result_file_path: usbr.ws_tester.result_file_path}
            });
        },
        ...
        display_run_dates:       function display_result_dates() {
            var html = '';
        ...
        display_results:            function display_results(response) {
            ...
            var application_type = (selected_tab == 'Test' ? $('#test_test_types option:selected').attr('application_type') : $('#review_test_types option:selected').attr('application_type'));

            $('#' + container_name).empty();
            
            if (response.status == 'success') {        
                if (response.data.length > 0) {
                    var response_columns = ['Expected', 'Comparison Method', 'Actual', 'Comparison Result'];
                    ...
                    for (var i = 0, length1 = data.length; i < length1; i++) {
                        // Info for entire test

                        html = '<table id="table_test_' + (i + 1) + '">';
                        html += '    <caption>Test #' + (i+1) + ' : ' + data[i].title + '</caption>';
                        html += '    <thead><tr><th>&nbsp;</th></tr></thead><tbody></tbody></table>';
                        ...
                        if (application_type == 'web_site') {
                            ...
                            for (var j = 0, length2 = data[i].steps.length; j < length2; j++) {
                                ...
                                // Request
                                html += '<table id="table__' + (i + 1) + '_request_' + (j + 1) + '">';
                                html += '<caption>Step #' + (j+1) + ' Request</caption>';
                                html += '<thead>><tr><th>&nbsp;</th></tr></thead><tbody>';

                                if (data[i].steps[j].request.variety == 'uri') {
                                    html += '        <tr><td>Method</td><td>' + html_encode(data[i].steps[j].request.method_name) + '</td></tr>';
                                    html += '        <tr><td>URI</td><td>' + html_encode(data[i].steps[j].request.uri) + '</td></tr>';
                                }
                                else if (data[i].steps[j].request.variety == 'form') {
                                    html += '        <tr><td>Form</td><td>' + html_encode(data[i].steps[j].request[data[i].steps[j].request.locator]) + '</td></tr>';
                                }
                                html += '    </tbody>';
                                html += '</table>';
                                ...
                                html += '<caption>Step #' + (j+1) + ' Response</caption>';
                                html += '<thead><tr><th>&nbsp;</th>';
                                    ...
                                    for (var k = 0, length3 = data[i].steps[j].expected[key].length; k < length3; k++) {
                                        ...
                                        for (var m = 0, length4 = response_columns.length; m < length4; m++) {
                                            ...
                                            if (response_columns[m] == 'Expected') {
                                                ...
                                                html += '<td>' + html_encode(data[i].steps[j].actual[key]) + '</td>';
                                            }
                                            else {
                            ...
                            // Request
                            html += '<table id="table__' + (i + 1) + '_request_' + (j + 1) + '">';
                            html += '<caption>Test #' + (i+1) + ' Request</caption>';
                            html += '<thead>><tr><th>&nbsp;</th></tr></thead><tbody>';
                            html += '        <tr><td>Method</td><td>' + html_encode(data[i].request.method_name) + '</td></tr>';
                            html += '        <tr><td>URI</td><td>' + html_encode(data[i].request.uri) + '</td></tr>';
                            html += '    </tbody>';
                            html += '</table>';
                            ...
                            for (var key in data[i].expected) {
                                for (var k = 0, length3 = data[i].expected[key].length; k < length3; k++) {
                                    html += '<tr><td>' + key + '</td>';
                                    for (var m = 0, length4 = response_columns.length; m < length4; m++) {
                                        var variety = data[i].expected[key][k].variety;

                                        if (response_columns[m] == 'Expected') {
                                            if (variety == 'logical') {
                                                html += '<td>' + html_encode(data[i].expected[key][k].operator) + ' ' + html_encode(data[i].expected[key][k].value) + '</td>';
                                            }
                                            else if (variety == 'xpath' || variety == 'jsonpath') {
                                                html += '<td>' + html_encode(data[i].expected[key][k].expression) + ' : ' + html_encode(data[i].expected[key][k].value) + '</td>';
                                            }
                                            else if (variety == 'image') {
                                                var split_result = data[i].expected[key][k].value.split("=");
                                                html += '<td>' + html_encode(split_result[0]) + ' : ' + html_encode(split_result[1]) + '</td>';
                                            }
                                            else if (variety == 'regexp') {
                                                html += '<td>' + html_encode(data[i].expected[key][k].value) + '</td>';
                                            }
                                        }
                                        else if (response_columns[m] == 'Comparison Method') {
                                            html += '<td>' + html_encode(variety) + '</td>';
                                            ...
                                            html += '<td>' + html_encode(data[i].actual[key]) + '</td>';
                                        }
                                        else {
                                            var status = data[i].comparison[key][k].status;
                                            var color = (status === 'PASS' ? 'green' : 'red');
                                            html += '<td style="background-color:' + color + '">' + status + '</td>';
                                        }
                                    }
        ...
        usbr.ws_tester.execute_tests()
            .done(function(response) {
                ...
                if (response.status == 'success') {
                    ...
                    // Remove everything in the results container.
                    $('#' + container_name).empty();                
                    ...
                    usbr.ws_tester.result_info.push({application: application, path: response.data.result_file_path, test_type: test_type, epoch: epoch});
                }
                else {
                    $.pnotify({
                        title: 'Error',
